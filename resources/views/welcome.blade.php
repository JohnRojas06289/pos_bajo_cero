            ...
            @forelse($featuredProducts as $product)
                <div class="col-lg-3 col-md-6 mb-5">
                    <div class="card product-card h-100 border-0">
                        ...
                        {{-- Imagen del producto (usa el accessor correcto) --}}
                        @if($product->img_path)
                            <img class="card-img-top" src="{{ $product->image_url }}" alt="{{ $product->nombre }}" style="height:300px;object-fit:cover;" loading="lazy" />
                        @else
                            <div style="height:300px;background:#222;display:flex;align-items:center;justify-content:center;">
                                <i class="fas fa-vest fa-3x text-muted"></i>
                            </div>
                        @endif
